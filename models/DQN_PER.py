...
class DQN_PER:
    ...
    @staticmethod
    def run(env_name):
        episode_durations, max_positions, max_positions_per_ep = DQN_PER(env_name).__run_episodes()
    ...
    def __train(self):
        ...
        with torch.no_grad():
            ...
            td_error = torch.abs(target - q_val).detach()
...
def compute_td_error(model, state, action, reward, next_state, done, discount_factor):
    state = torch.tensor(state, dtype=torch.float)
    action = torch.tensor(action, dtype=torch.int64)
    next_state = torch.tensor(next_state, dtype=torch.float)
    reward = torch.tensor(reward, dtype=torch.float)
